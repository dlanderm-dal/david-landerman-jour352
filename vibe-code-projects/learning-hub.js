/* ============================================
   LEARNING HUB — Navbar + Inline Comments + Quick Fix Sidebar
   Simplified UX: comments on page, sidebar for quick fixes only.
   ============================================ */
...
(function () {
  ...
  // ========== CONFIG ==========
  var STORAGE_KEY = 'learningHubComments';
  var INSTRUCTIONS_KEY = 'learningHubCustomInstructions';

  // ========== DETECT PAGE ==========
  var pathParts = window.location.pathname.split('/');
  var currentFile = pathParts[pathParts.length - 1] || 'index.html';
  var isInSubfolder = window.location.pathname.indexOf('/AI_study_guides/') !== -1;
  var prefix = isInSubfolder ? '../' : '';
  var guideTitle = document.title || currentFile;
  var filePath = isInSubfolder
    ? 'vibe-code-projects/AI_study_guides/' + currentFile
    : 'vibe-code-projects/' + currentFile;

  // ========== GUIDE DATA (for search) ==========
  var allGuides = [
    { file: 'box_model_visual.html', title: 'Box Model Visual Guide', cat: 'Layout' },
    { file: 'positioning_study_guide.html', title: 'CSS Positioning', cat: 'Layout' },
    { file: 'display_flexbox_cards_guide.html', title: 'Display, Flexbox & Cards', cat: 'Layout' },
    { file: 'whole_page_architecture.html', title: 'Whole Page Architecture', cat: 'Layout' },
    { file: 'bootstrap-breakpoints-guide.html', title: 'Bootstrap Breakpoints', cat: 'Bootstrap' },
    { file: 'bootstrap_percentage_study_guide.html', title: 'Bootstrap Percentages', cat: 'Bootstrap' },
    { file: 'concept1_bootstrap_version_fixed.html', title: 'Bootstrap Core Concepts', cat: 'Bootstrap' },
    { file: 'naming-conventions-guide.html', title: 'Naming Conventions', cat: 'Text' },
    { file: 'naming-conventions-guide-2.html', title: 'Naming Conventions (Part 2)', cat: 'Text' },
    { file: 'text-layouts-tutorial.html', title: 'Text Layouts Tutorial', cat: 'Text' },
    { file: 'text-layouts-tutorial-2.html', title: 'Text Layouts (Part 2)', cat: 'Text' },
    { file: 'layout_debugging_guide.html', title: 'Layout Debugging', cat: 'Debug' },
    { file: 'layout_debugging_guide-3.html', title: 'Layout Debugging (Advanced)', cat: 'Debug' },
    { file: 'float-demo.html', title: 'Float Demo', cat: 'Demos' },
    { file: 'float-demo-2.html', title: 'Float Demo (Part 2)', cat: 'Demos' },
    { file: 'carousel-text-example.html', title: 'Carousel & Text Example', cat: 'Demos' },
    { file: 'Claude Join page copy.html', title: 'Claude Join Page', cat: 'Demos' }
  ];
  ...
  var navHTML = '' +
    '<nav class="lh-navbar" id="lhNavbar">' +
      '<a href="' + prefix + 'index.html" class="lh-nav-brand">Learning Hub</a>' +
      ...
      '<div class="lh-search-wrap">' +
        '<span class="lh-search-icon">&#x1F50D;</span>' +
        '<input type="text" class="lh-search-input" id="lhSearchInput" placeholder="Search guides..." autocomplete="off">' +
        '<div class="lh-search-results" id="lhSearchResults"></div>' +
      '</div>' +
      '<div class="lh-nav-dropdown-wrap">' +
        '<button class="lh-nav-dropdown-btn" id="lhCommentsDropBtn">Comments <span class="lh-badge" id="lhCommentBadge"' + (commentCount > 0 ? '' : ' style="display:none;"') + '>' + commentCount + '</span></button>' +
        ...
        '</div>' +
      '</div>' +
      '<button class="lh-nav-export" id="lhBatchExport" title="Copy all comments on this page as a batch prompt">Export Comments</button>' +
    '</nav>';
  ...
  var searchInput = document.getElementById('lhSearchInput');
  var searchResults = document.getElementById('lhSearchResults');
  ...
  showResolvedBtn.addEventListener('click', function () {
    ...
    if (showingResolved) {
      this.textContent = 'Hide Resolved Comments';
      renderResolvedComments();
    } else {
      this.textContent = 'Show Resolved Comments';
  ...
  function renderResolvedComments() {
    ...
    if (resolved.length === 0) { showToast('No resolved comments on this page.'); showingResolved = false; showResolvedBtn.textContent = 'Show Resolved Comments'; return; }

    var overlay = document.createElement('div');
    overlay.id = 'lhResolvedOverlay';
    overlay.className = 'lh-resolved-overlay';

    var html = '<div class="lh-resolved-panel"><h3>Resolved Comments</h3>';
  ...
  document.addEventListener('click', function (e) {
    ...
    if (link && !e.target.closest('.lh-navbar') && !e.target.closest('.lh-search-results')) {
      e.preventDefault();
      e.stopPropagation();
    }
  }, true);
  ...
  document.addEventListener('mousedown', function (e) {
    ...
    if (activeCommentPopup && !activeCommentPopup.contains(e.target) && !(e.target.closest && e.target.closest('.lh-comment-marker'))) {
      removeCommentPopup();
    }
  });
  ...
  function showFloatingBar(mouseEvent, selectedText) {
    ...
    // Ensure the bar doesn't go above the navbar (56px) + scroll offset
    var minTop = window.scrollY + 60;
  ...
  function captureDOMContext() {
    try {
      ...
      // Build a CSS-like path: tag#id.class > tag.class (up to 3 levels)
      var path = [];
      var current = el;
  ...
  var LATEST_BATCH_KEY = 'learningHubLatestBatch_' + currentFile;
  ...
  function scanForImplemented() {
    ...
    all.forEach(function (c) {
      ...
      // If we already knew about this batch, clear the isLatestBatch flag (highlight only on first load)
      if (!isNewBatch && c.isLatestBatch) {
        c.isLatestBatch = false;
        changed = true;
      }
      // Mark all replies on implemented comments as handled so they don't re-export
  ...
  // so the marker can be placed inside it instead of on the page body.
  function findTextPosition(searchText) {
    ...
    while (node = walker.nextNode()) {
      ...
      if (node.textContent.indexOf(snippet) !== -1) {
        ...
        var container = null;
        if (el) {
          container = el.closest('.queue-item') || el.closest('.lh-entry-item') || el.closest('[data-dynamic-container]');
        }
  ...
  function renderCommentMarkers() {
    ...
    var hasTabs = !!document.querySelector('.tab-btn');

    // Build a set of all current tab names for orphan detection
    var allTabNames = [];
    document.querySelectorAll('.tab-btn').forEach(function (btn) { allTabNames.push(btn.textContent.trim()); });
  ...
  function showCommentDetail(comment, markerEl) {
    ...
    if (comment.image) {
      ...
      commentImgEl.addEventListener('click', function () { window.open(comment.image); });
    }
    ...
    replies.forEach(function (r) {
      ...
      replyDiv.innerHTML = '<span class="lh-thread-reply-text">' + escapeHtml(r.text) + '</span><span class="lh-thread-reply-time">' + new Date(r.timestamp).toLocaleString() + '</span>';
    ...
    replyBtn.addEventListener('click', function () {
      ...
      if (target) {
        ...
        target.replies.push({ text: text, timestamp: new Date().toISOString() });
        saveAllComments(all);
    ...
    replyInput.addEventListener('keydown', function (e) { if (e.key === 'Enter') replyBtn.click(); });
  ...
  function updateBadge() {
    ...
    // Count by status — include replies in totals
    var counts = { active: 0, implemented: 0, resolved: 0 };
    var totalItems = 0; // parents + replies
    ...
    pageComments.forEach(function (c) {
      ...
      unresolvedWithReplies++; // the parent
    ...
    // Export button badge: active parents + unhandled replies on any comment
  ...
  quickFixBtn.addEventListener('click', function () {
    ...
    var promptFilePath = 'vibe-code-projects/AI_study_guides/prompts/' + currentFile.replace('.html', '') + '.prompt.txt';

    var prompt = 'FIRST (if you haven\'t already in this conversation): Read `vibe-code-projects/ARCHITECTURE.md` — it is the single source of truth for this project. Follow its conventions.\n\n';
    prompt += 'THIS IS A QUICK FIX from the Learning Hub annotation system.\n\n';
  ...
  batchExportBtn.addEventListener('click', function () {
    ...
    if (exportItems.length === 0) {
      showToast('No new comments to export.');
      return;
    }
    ...
    var prompt = 'FIRST (if you haven\'t already in this conversation): Read `vibe-code-projects/ARCHITECTURE.md` — it is the single source of truth for this project. Follow its conventions.\n\n';
    prompt += 'THIS IS AN EXPORTED COMMENT BATCH from the Learning Hub annotation system.\n\n';
    prompt += 'Edit the file `' + filePath + '`.\n\n';
    ...
    exportItems.forEach(function (item, i) {
      ...
      if (item.type === 'follow-up') {
        ...
        if (c.replies && c.replies.length > 0) {
          ...
          c.replies.forEach(function (r) { prompt += '- ' + r.text + '\n'; });
          prompt += '\n';
        }
      }
    ...
    // Top-level comments stay active until Claude Code implements them (via lh-implemented marker).
    // But follow-up replies on already-implemented comments get marked as sent after export,
    // because they won't get their own implementation marker — they piggyback on the parent.
    var all = getAllComments();
    all.forEach(function (c) {
      if (c.status === 'implemented' && c.replies) {
    ...
    copyText(prompt, batchExportBtn, 'Export Comments');
  });
  ...
  searchInput.addEventListener('input', function () {
    ...
    if (query.length < 2) { searchResults.classList.remove('lh-visible'); return; }

    searchTimeout = setTimeout(function () {
      var results = allGuides.filter(function (g) {
        return g.title.toLowerCase().indexOf(query) !== -1 ||
               g.cat.toLowerCase().indexOf(query) !== -1;
      });

      if (results.length === 0) {
        searchResults.innerHTML = '<div class="lh-search-no-results">No guides found for "' + escapeHtml(query) + '"</div>';
      } else {
        var html = '';
        results.forEach(function (g) {
          var guidePath = isInSubfolder ? '' : 'AI_study_guides/';
          var href = guidePath + encodeURIComponent(g.file);
          html += '<a class="lh-search-result-item" href="' + href + '">';
          html += '<div class="lh-search-result-title">' + escapeHtml(g.title) + '</div>';
          html += '<div class="lh-search-result-snippet">' + escapeHtml(g.cat) + '</div>';
          html += '</a>';
        });
        searchResults.innerHTML = html;
      }
      searchResults.classList.add('lh-visible');
    }, 150);
  });

  searchInput.addEventListener('focus', function () {
    if (this.value.trim().length >= 2) searchResults.classList.add('lh-visible');
  });

  document.addEventListener('click', function (e) {
    if (!searchInput.contains(e.target) && !searchResults.contains(e.target)) {
      searchResults.classList.remove('lh-visible');
    }
  });
  ...
  // ========== STICKY TAB BAR ==========
  // Find the container holding tab buttons and make it sticky below the navbar
  var tabBtns = document.querySelectorAll('.tab-btn');
  if (tabBtns.length > 0) {
    var tabContainer = tabBtns[0].parentElement;
    if (tabContainer) {
      // Walk up to the nav element if the buttons are inside a wrapper
      var tabNav = tabContainer.closest('nav') || tabContainer;
      tabNav.classList.add('lh-sticky-tabs');
    }
  }
  ...
  function copyText(text, btn, originalLabel) {
    function showCopied() {
      // Save and restore child elements (like badge spans) — don't use textContent which destroys them
      var savedChildren = [];
  ...
  // ========== TRACK LAST OPENED ==========
  if (isInSubfolder || (currentFile !== 'index.html' && currentFile !== 'new-topic.html' && currentFile !== 'change-file.html' && currentFile !== 'convert-guide.html')) {
    var lastInfo = {
      title: guideTitle,
      desc: '',
      href: isInSubfolder ? 'AI_study_guides/' + currentFile : currentFile,
      thumb: isInSubfolder ? 'thumbnails/' + currentFile.replace('.html', '') + '.png' : ''
    };
    localStorage.setItem('learningHubLastOpened', JSON.stringify(lastInfo));
  }

  // ========== STARTUP ==========
  // Scan after a short delay to ensure the full HTML (including lh-implemented comment) is available
  renderCommentMarkers();
  updateBadge();
  setTimeout(function () {
    scanForImplemented();
